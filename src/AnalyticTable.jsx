...
function AnaliticTable({data}){
    ...
    const handleScroll = () => {
        if (tableContainerRef.current){
            ...
            setShowScrollTop(scrollTop > 200);    // кнопка показывается, если прокрутка ниже 200px
        }
    };

    const scrollToTop = () => {
        if (tableContainerRef.current){ // проверяем существование таблицы
            tableContainerRef.current.scrollTo({ // устанавливает позицию скролла
    ...
    const handleMouseDown = (e, columnName, headerElement) => {
        ...
        resizeInfo.current ={
            ...
            startX: e.clientX, // местонахождение курсора
            startWidth: headerElement.offsetWidth, // хранит ширину дом-элемента
            headerRef: headerElement
        };
        // делаем движение мышью
    ...
    const handleMouseMove = (e) => {
        const info = resizeInfo.current; // сохраняем текущее положение
        if (!info.isResizing) return;

        const deltaX = e.clientX - info.startX;
        const newWidth = Math.max(100, info.startWidth + deltaX);

        // меняем DOM напрмую, чтобы не было рендеринга, через useRef, без вызова setState
        if(info.headerRef){
    ...
    const handleMouseUp = () => {
        ...
        if (info.isResizing && info.currentColumn){
            // только ТЕПЕРЬ фиксируем в стейт финальную ширину
    ...
    // ссылки на th (пойдут в mouseDown)
